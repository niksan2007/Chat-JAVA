        ...
        private String requestAndAddingUser(Connection connection) {
            while (true) {
                try {
                    ...
                    //если тип сообщения USER_NAME и имя не null и не пустое, то
                    if (responseMessage.getTypeMessage() == MessageType.USER_NAME && userName != null && !userName.isEmpty()){
                        //добавляем или обновляем в БД пользователя
                        queryUsers.insertOrUpdate(userName);
                    }
